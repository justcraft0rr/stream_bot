import type { TokenProvider } from "../core/auth/TokenProvider.mjs";
import { WorkflowsClient } from "../api/resources/workflows/client/Client.mjs";
import { PipedreamClient } from "../Client.mjs";
export type PipedreamClientOpts = Pick<PipedreamClient.Options, "baseUrl" | "clientId" | "clientSecret" | "headers" | "projectEnvironment"> & {
    /**
     * The unique identifier for the project. This field is required, passed
     * either explicitly or by setting the `PIPEDREAM_PROJECT_ID` environment
     * variable.
     */
    projectId?: string;
    /**
     * Optional token provider for authentication.
     */
    tokenProvider?: TokenProvider;
    /**
     * Optional custom domain for workflow execution.
     */
    workflowDomain?: string;
};
export declare class Pipedream extends PipedreamClient {
    private _workflowDomain?;
    private _workflows;
    constructor(opts?: PipedreamClientOpts);
    /**
     * Returns an access token that can be used to authenticate API requests
     *
     * @returns A promise that resolves to the access token string.
     */
    get rawAccessToken(): Promise<string>;
    get workflows(): WorkflowsClient;
}
