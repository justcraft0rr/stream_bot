import type { BaseClientOptions, BaseRequestOptions } from "../../../../BaseClient.js";
import { type NormalizedClientOptionsWithAuth } from "../../../../BaseClient.js";
import * as core from "../../../../core/index.js";
import * as Pipedream from "../../../index.js";
export declare namespace ProxyClient {
    type Options = BaseClientOptions;
    interface RequestOptions extends BaseRequestOptions {
    }
}
export declare class ProxyClient {
    protected readonly _options: NormalizedClientOptionsWithAuth<ProxyClient.Options>;
    /**
     * Precompiled regex to check if an Accept header accepts JSON responses.
     * Matches: application/json (with or without parameters)
     */
    private static readonly JSON_ACCEPT_REGEX;
    /**
     * Precompiled regex to check if a Content-Type header is JSON.
     * Matches: application/json (with or without parameters like charset)
     */
    private static readonly JSON_CONTENT_TYPE_REGEX;
    constructor(_options: ProxyClient.Options);
    /**
     * Transform headers by prefixing each key with 'x-pd-proxy-'
     */
    private transformProxyHeaders;
    /**
     * Determine the response type based on the `Accept` header provided by the
     * caller. Note that the caller's headers must have already been transformed
     * via `transformProxyHeaders`.
     */
    private getResponseType;
    /**
     * Check if the response is JSON based on content-type header
     */
    private isJsonResponse;
    /**
     * Forward an authenticated GET request to an external API using an external user's account credentials
     *
     * @param {Pipedream.ProxyGetRequest} request
     * @param {ProxyClient.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link Pipedream.TooManyRequestsError}
     *
     * @example
     *     await client.proxy.get({
     *         url: "https://api.example.com/endpoint",
     *         externalUserId: "external_user_id",
     *         accountId: "account_id",
     *         params: { key: "value" },
     *         headers: { "X-Custom-Header": "value" }
     *     })
     */
    get(request: Pipedream.ProxyGetRequest, requestOptions?: ProxyClient.RequestOptions): core.HttpResponsePromise<Pipedream.ProxyResponse | core.BinaryResponse | undefined>;
    private __get;
    /**
     * Forward an authenticated POST request to an external API using an external user's account credentials
     *
     * @param {Pipedream.ProxyPostRequest} request
     * @param {ProxyClient.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link Pipedream.TooManyRequestsError}
     *
     * @example
     *     await client.proxy.post({
     *         url: "https://api.example.com/endpoint",
     *         externalUserId: "external_user_id",
     *         accountId: "account_id",
     *         body: { "key": "value" },
     *         params: { key: "value" },
     *         headers: { "X-Custom-Header": "value" }
     *     })
     */
    post(request: Pipedream.ProxyPostRequest, requestOptions?: ProxyClient.RequestOptions): core.HttpResponsePromise<Pipedream.ProxyResponse | core.BinaryResponse | undefined>;
    private __post;
    /**
     * Forward an authenticated PUT request to an external API using an external user's account credentials
     *
     * @param {Pipedream.ProxyPutRequest} request
     * @param {ProxyClient.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link Pipedream.TooManyRequestsError}
     *
     * @example
     *     await client.proxy.put({
     *         url: "https://api.example.com/endpoint",
     *         externalUserId: "external_user_id",
     *         accountId: "account_id",
     *         body: { "key": "value" },
     *         params: { key: "value" },
     *         headers: { "X-Custom-Header": "value" }
     *     })
     */
    put(request: Pipedream.ProxyPutRequest, requestOptions?: ProxyClient.RequestOptions): core.HttpResponsePromise<Pipedream.ProxyResponse | core.BinaryResponse | undefined>;
    private __put;
    /**
     * Forward an authenticated DELETE request to an external API using an external user's account credentials
     *
     * @param {Pipedream.ProxyDeleteRequest} request
     * @param {ProxyClient.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link Pipedream.TooManyRequestsError}
     *
     * @example
     *     await client.proxy.delete({
     *         url: "https://api.example.com/endpoint",
     *         externalUserId: "external_user_id",
     *         accountId: "account_id",
     *         params: { key: "value" },
     *         headers: { "X-Custom-Header": "value" }
     *     })
     */
    delete(request: Pipedream.ProxyDeleteRequest, requestOptions?: ProxyClient.RequestOptions): core.HttpResponsePromise<Pipedream.ProxyResponse | core.BinaryResponse | undefined>;
    private __delete;
    /**
     * Forward an authenticated PATCH request to an external API using an external user's account credentials
     *
     * @param {Pipedream.ProxyPatchRequest} request
     * @param {ProxyClient.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link Pipedream.TooManyRequestsError}
     *
     * @example
     *     await client.proxy.patch({
     *         url: "https://api.example.com/endpoint",
     *         externalUserId: "external_user_id",
     *         accountId: "account_id",
     *         body: { "key": "value" },
     *         params: { key: "value" },
     *         headers: { "X-Custom-Header": "value" }
     *     })
     */
    patch(request: Pipedream.ProxyPatchRequest, requestOptions?: ProxyClient.RequestOptions): core.HttpResponsePromise<Pipedream.ProxyResponse | core.BinaryResponse | undefined>;
    private __patch;
}
